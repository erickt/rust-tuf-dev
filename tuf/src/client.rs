    ...
    T: PathTranslator,
{
    ...
    #[cfg(test)]
    pub(crate) fn local_repo(&self) -> &L {
        self.local.inner()
    }

    /// Return a reference to the remote repository.
    #[cfg(test)]
    pub(crate) fn remote_repo(&self) -> &R {
        self.remote.inner()
    }

    /// Return a reference to the remote repository.
    #[cfg(test)]
    pub(crate) fn remote_repo_mut(&mut self) -> &mut R {
        self.remote.inner_mut()
    }
    ...
    R: RepositoryProvider<D>,
{
    ...
    if tuf.trusted_root().consistent_snapshot() {
        ...
        remote.fetch_target(&target, &target_description).await
    } else {
        remote.fetch_target(target, &target_description).await
    }
}
...
mod test {
    ...
    fn constructor_succeeds_with_missing_metadata() {
        block_on(async {
            ...
            let _metadata2 = RepoBuilder::new(client.remote_repo_mut())
                .root_keys(vec![&KEYS[0]])
                .targets_keys(vec![&KEYS[0]])
                .snapshot_keys(vec![&KEYS[0]])
                .timestamp_keys(vec![&KEYS[0]])
    ...
    fn constructor_succeeds_with_expired_metadata() {
        block_on(async {
            ...
            // An update should succeed.
            let _metadata3 = RepoBuilder::new(client.remote_repo_mut())
                .root_keys(vec![&KEYS[0]])
                .targets_keys(vec![&KEYS[0]])
                .snapshot_keys(vec![&KEYS[0]])
                .timestamp_keys(vec![&KEYS[0]])
    ...
    async fn root_chain_update(consistent_snapshot: bool) {
        ...
        let metadata2 = RepoBuilder::new(client.remote_repo_mut().inner_mut())
        ...
        let metadata3 = RepoBuilder::new(client.remote_repo_mut().inner_mut())
    ...
    fn update_fails_if_cannot_write_to_repo() {
        block_on(async {
            ...
            let _ = RepoBuilder::new(client.remote_repo_mut())
                .root_keys(vec![&KEYS[0]])
                .targets_keys(vec![&KEYS[0]])
                .snapshot_keys(vec![&KEYS[0]])
                .timestamp_keys(vec![&KEYS[0]])
            ...
            client.local_repo().fail_metadata_stores(true);
